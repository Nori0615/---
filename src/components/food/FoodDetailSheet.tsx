  ...
  return (
    <Sheet open={Boolean(food)} title={food.name} onClose={() => selectFood(undefined)}>
      <div className="grid gap-5">
        <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-700" style={{ backgroundColor: food.color }}>
          <div className="flex items-center gap-3">
            <div className="grid h-14 w-14 place-items-center rounded-lg bg-white/80 text-3xl">{food.icon}</div>
            <div>
              <p className="text-sm font-semibold text-slate-600">{food.category}</p>
              <h3 className="text-2xl font-bold text-slate-900">{food.name}</h3>
            </div>
          </div>
          <div className={`mt-4 rounded-lg border px-3 py-2 text-sm font-semibold ${statusClass(status.state)}`}>{status.label}</div>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-lg bg-slate-50 p-3 transition-colors dark:bg-slate-950">
          ...
          <div className="rounded-lg bg-slate-50 p-3 transition-colors dark:bg-slate-950">
            ...
            <p className="mt-2 text-lg font-semibold text-slate-900 dark:text-slate-50">{area?.name ?? "未設定"}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-lg bg-white p-3 shadow-sm transition-colors dark:bg-slate-950">
            <p className="text-xs font-bold text-slate-500 dark:text-slate-400">消費期限</p>
            <p className="mt-1 font-semibold text-slate-900 dark:text-slate-50">{displayDate(food.expireDate)}</p>
          </div>
          <div className="rounded-lg bg-white p-3 shadow-sm transition-colors dark:bg-slate-950">
            <p className="text-xs font-bold text-slate-500 dark:text-slate-400">賞味期限</p>
            <p className="mt-1 font-semibold text-slate-900 dark:text-slate-50">{displayDate(food.bestBeforeDate)}</p>
          </div>
        </div>

        {food.memo ? (
          <div className="rounded-lg bg-cyan-50 p-4 text-sm leading-6 text-slate-700 transition-colors dark:bg-teal-950/40 dark:text-slate-300">
            <p className="mb-1 font-bold text-slate-900 dark:text-slate-50">メモ</p>
            {food.memo}
          </div>
        ) : null}
