...
export function QuickAddFood() {
  ...
  return (
    <section className="rounded-[1.25rem] border border-slate-200/80 bg-white p-5 shadow-[0_16px_40px_rgba(15,23,42,0.08)] transition-colors dark:border-slate-800 dark:bg-slate-900/90">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.16em] text-teal-700 dark:text-teal-300">Quick</p>
          <h2 className="text-lg font-black text-slate-900 dark:text-slate-50">すぐ追加</h2>
        </div>
        ...
        {favorites.length > 0 ? (
          favorites.map((food) => (
            <button
              ...
              className="focus-ring inline-flex shrink-0 items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-black text-slate-800 transition-colors hover:bg-white dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:hover:bg-slate-800"
            >
